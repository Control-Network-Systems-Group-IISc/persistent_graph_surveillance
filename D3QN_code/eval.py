...
def eval_graph_pol(args):
  ...
  list_of_iters_to_test = [(i_episode+1)*EVAL_INTERVAL*MAX_SIM_TIME \
    ...
    while (t__ in range(HORIZON)):
      ...
      if BEAM_SEARCH and SEQ_ACT:
        ...
        action_duration = env_utils.env_step(next_graph_nbs, t_rem, dqn_agent_, 
                                             next_agent_occ_dict_, explore=False, seq_flag=SEQ_ACT)
        ...
        if SEQ_ACT:
          ...
          prev_location = next_graph_.nodes[NUM_NODES]['agents_occ'][0]
          ...
          next_location = next_graph_.nodes[NUM_NODES]['agents_occ'][0]

          num_time_step_skip = \
                  next_graph_[prev_location][next_location]['travel_time']

          t__ += num_time_step_skip
  ...
  G = nx.read_gpickle(f"{graph_read_path}/test_instances/instance_1_multi.gpickle")
  ...
  for i_ in range(1, NUM_TEST_INSTANCES+1):
    temp_graph_obj = nx.read_gpickle(f"{graph_read_path}/test_instances/instance_{i_}_multi.gpickle")
  ...
  for iter_ in list_of_iters_to_test:
    ...
    for i_ in range(1, NUM_TEST_INSTANCES+1):
      eval_graph = nx.read_gpickle(
                    f"{graph_read_path}/test_instances/instance_{i_}_multi.gpickle")
