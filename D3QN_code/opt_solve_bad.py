...
class solve_main():
  ...
  def __init__(self, graph_obj, agents_obj, prev_plan, init_battery, max_bat,
    ...
    self.V = list(filter(lambda node: (\
        not (self.graph.nodes[node]['true_node'])), self.graph.nodes()))
    ...
    self.model.addCons(self.obj_act == quicksum(\
        self.graph.nodes[j]['priority'] * self.D[j][t+1] for j in \
                        ...
                        # quicksum(self.G[t_] for t_ in self.time_inst_list[1:]))
    ...
    # self.model.tightenVarUbGlobal(self.obj, self.opt_bound)

    # self.model.setObjlimit(self.opt_bound)
    ...
    for i_ in self.V_not_C:
      self.model.addCons(self.D[i_][0] == (self.D_j_0[i_] + self.f_i(i_, 0)) * (1 - quicksum(self.x_hat[i_][k_][0] for k__ in self.A)) )
      for t_ in self.time_inst_list[:-1]:
        # m = min(1, quicksum(...))

        # self.model.addCons(self.D[i_][t_+1] == (self.D[i_][t_] + self.f_i(i_, t_))*(1 - quicksum(self.x[i_][k__][t_] for k__ in self.A))) 
    ...
    for k in self.A:
      for i in self.V:
        # if ((-1 + self.graph[i][j_]['travel_time']) in self.time_inst_list):
        non_neighbours_list = []
        for _ in self.V:
          if _ not in list(self.graph.neighbors(i)):
            non_neighbours_list.append(_) 

        self.model.addCons(quicksum(\
          self.x[j_][k][-1 + self.graph[i][j_]['travel_time']] \
          for j_ in self.graph.neighbors(i)) >= self.x_hat[i][k][0])

        for j in self.graph.neighbors(i):
          for t_ in range(1, self.graph[i][j]['travel_time']):
            self.model.addCons(quicksum(\
              self.x[j_][k][-1 + t_] for j_ in self.V) <= \
              1 - self.x[j][k][-1 + self.graph[i][j]['travel_time']])

        for t_ in self.time_inst_list:
          # if t_ + self.graph[i][j_]['travel_time'] in self.time_inst_list:
          self.model.addCons(quicksum(\
            self.x[j_][k][t_ + self.graph[i][j_]['travel_time']] \
            if (t_ + self.graph[i][j_]['travel_time'] in self.time_inst_list) \
            else 0 \
            for j_ in self.graph.neighbors(i)) >= self.x[i][k][t_])

          for j in self.graph.neighbors(i):
            for t__ in range(1, self.graph[i][j]['travel_time']):
              if (t_ + t__ in self.time_inst_list) and \
                  (t_ + self.graph[i][j]['travel_time'] in self.time_inst_list):
                self.model.addCons(quicksum(\
                  self.x[j_][k][t_ + t__] for j_ in self.V) <= \
                  1 - self.x[j][k][t_ + self.graph[i][j]['travel_time']])


        '''self.model.addCons(quicksum(\
          self.x[j_][k][-1 + t__] if -1 + t__ in self.time_inst_list else 0 \
          for j_ in self.graph.neighbors(i) \
          for t__ in range(1, self.graph[i][j_]['travel_time'])) \
           <= 1 - self.x_hat[i][k][0])
        ...
        for t_ in self.time_inst_list:
          self.model.addCons(quicksum(\
            self.x[j_][k][t_ + self.graph[i][j_]['travel_time']] \
            if t_ + self.graph[i][j_]['travel_time'] in self.time_inst_list else 0 \
            for j_ in self.graph.neighbors(i)) >= self.x[i][k][t_])

          self.model.addCons(quicksum(\
            self.x[j_][k][t_ + t__] if t_ + t__ in self.time_inst_list else 0 \
            for j_ in self.graph.neighbors(i) \
            for t__ in range(1, self.graph[i][j_]['travel_time'])) \
             <= 1 - self.x[i][k][t_])


          for j__ in self.graph.neighbors(i):
            if ((t_ + self.graph[i][j__]['travel_time']) in self.time_inst_list):
              self.model.addCons(quicksum(\
                self.x[j_][k][t_ + self.graph[i][j__]['travel_time']] \
                for j_ in non_neighbours_list) <= 1 - self.x[i][k][t_])

            if self.graph[i][j__]['travel_time'] > 1:
              self.model.addCons(quicksum(\
                self.x[j_][k][t_ + t__] if t_ + t__ in self.time_inst_list else 0 \
                for t__ in range(1, self.graph[i][j__]['travel_time']) \
                for j_ in non_neighbours_list) \
                 <= 1 - self.x[i][k][t_]) '''
